...
class operations:
    ...
    def __init__(self, cursor, conn) -> None:
        ...
        self.connection_obj = cursor
        self.cursor_obj = conn
    ...
    def getSMS(self, sms, sender="PAYTMB"):
    ...
    def matchRegex(self) -> int:
        ...
        codes = self.cursor_obj.execute(
            f"""select rowid,* from sender where organisationName in (select organisationName from sender where code="{self.sender}");"""
        )

        for rowid, code, org in codes.fetchall():
            print("Code: ", code)
            # get regex under each code
            reg = self.cursor_obj.execute(
                f"""select * from regex where sender="{code}";"""
            )
    ...
    def change_accounts_table(self, debit: bool, field_val: list, account_no: int):
        resp = self.cursor_obj.execute(
            f"""select balance,tag from accounts where accountNo={account_no}"""
        )
        ...
        # PROBLEM: ADD CONDITION for other tags
            ...
            self.cursor_obj.execute(
                command, (account_no, "Rs", balance, "BANK ACCOUNT")
            )
    ...
    def validate_data(self, field_val: list, debit: bool, tag: str) -> bool:
        ...
        if tag == "BANK TRANSFER":
            ...
            # check account no:
            if field_val[2] != None:
                check = self.cursor_obj.execute(
                    f"""select * from accounts where accountNo={field_val[2]}"""
                )

                # check if recipient is an account number
                if field_val[3].isnumeric():
                    check2 = self.cursor_obj.execute(
                        f"""select * from accounts where accountNo={field_val[3]}"""
                    )

                    # if recipient is not an account number then debit or credit
                    if check2.fetchone() == None and debit != None:
                        if debit:
                            self.total_expense += float(field_val[1])
                        else:
                            self.total_income += float(field_val[1])
        ...
        if field_val[4] and field_val[5] and field_val[6]:
            ...
            date = datetime.strptime(
                f"{field_val[4]}/{field_val[5]}/{field_val[6]}", "%d/%m/%y"
            )
    ...
    def main(self, sms, sender="PAYTMB"):
        self.getSMS(sms, sender)
        id = self.matchRegex()
        print(id)
        if id != -1:
            self.getFields(id)
